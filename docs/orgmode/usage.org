...
.. raw:: html
   ...
   <div class="cn-page-intro">
     ...
     edit responses, quality check in CI, browse with the TUI, convert with
...
.. list-table::
   ...
   * - Command
     ...
     - Notes
   * - ``doctor``
     - Probe pandoc, Vale, Harper, optional [jira] config
   ...
   * - ``check``
     - Quality check (prefix, sop, team sign-off)
...
.. grid:: 1 2 2 2
   ...
   .. grid-item-card:: Environment and corpus
      ...
      .. raw:: html
         ...
         <ul class="cn-cmd-list">
           ...
           <li>canonic check<span class="cn-cmd-hint">quality check · exit 1 on findings</span></li>
           <li>canonic lint --engine harper</li>
         </ul>

   .. grid-item-card:: Author and promote
      :class-card: sd-border-0
   ...
   .. grid-item-card:: Search and dedupe
      :class-card: sd-border-0
   ...
   .. grid-item-card:: Convert and free Jira
      :class-card: sd-border-0
...
Put free Jira settings in ``canonic.local.toml`` (gitignored) or ``canonic.toml``:
...
.. code:: shell
   ...
   canonic doctor
   canonic list
   # included demo (no scaffold required):
   canonic convert corpus/responses/resp-demo-shared-quota.md
   canonic check
   canonic lint --engine harper
   canonic tui
...
**Authentication (`[jira]` in canonic.toml / canonic.local.toml):**

+------------------------+--------------------------------------------------+
| Key                    | Role                                             |
+========================+==================================================+
| ``base_url``           | Required, e.g. Cloud Free instance URL           |
+------------------------+--------------------------------------------------+
| ``email`` +            | Basic auth (Jira Cloud Free API token)           |
| ``api_token``          |                                                  |
+------------------------+--------------------------------------------------+
| ``auth_header``        | Raw ``Authorization`` header; wins if set        |
