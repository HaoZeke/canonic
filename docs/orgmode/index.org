#+TITLE: ~canonic~
#+AUTHOR: [[https://rgoswami.me][Rohit Goswami]]
#+OPTIONS: toc:nil todo:nil num:nil author:nil title:nil ^:nil

#+begin_export rst
.. raw:: html

   <div class="cn-hero">
     <p class="cn-hero-eyebrow">markdown corpus · Jira convert · quality checks</p>
     <div class="cn-hero-brand">
       <img class="cn-hero-mark" src="_static/mark.svg" width="60" height="60" alt="" />
       <div>
         <p class="cn-hero-name">canonic</p>
         <p class="cn-hero-sub">canonical responses, versioned in git</p>
       </div>
     </div>
     <img class="cn-hero-logo cn-hero-logo-light" src="_static/logo.svg" width="320" height="60" alt="canonic" />
     <img class="cn-hero-logo cn-hero-logo-dark" src="_static/logo-dark.svg" width="320" height="60" alt="canonic" />
     <p class="cn-hero-tagline">Versioned canned-response corpus for Jira workflows - markdown under a shared configured prefix is the source of truth. Convert with pandoc, gate with quality checks, search and dedupe with Tantivy.</p>
     <div class="cn-hero-pills">
       <span>markdown corpus</span>
       <span>shared prefix</span>
       <span>pandoc → jira</span>
       <span>Tantivy BM25</span>
       <span>review before migrate</span>
     </div>
     <div class="cn-hero-cta">
       <a class="cn-btn cn-btn-primary" href="tutorial.html">Tutorial</a>
       <a class="cn-btn cn-btn-ghost" href="usage.html">Usage and CLI</a>
       <a class="cn-btn cn-btn-ghost" href="design.html">Design notes</a>
       <a class="cn-btn cn-btn-ghost" href="https://github.com/HaoZeke/canonic">GitHub</a>
     </div>
   </div>

Why canonic
===========

Shared support answers should live in git, not only in Jira comments.
**canonic** treats version-controlled markdown under ``corpus/responses/`` as the
source of truth: check quality before migration, convert with pandoc's Jira
writer, lint with Vale or in-process Harper, and search or dedupe with a local
Tantivy index.

Workflow at a glance
--------------------

.. raw:: html

   <ol class="cn-flow">
     <li>
       <span class="cn-flow-n">1</span>
       <span class="cn-flow-title">Author in markdown</span>
       <span class="cn-flow-desc">Shared prefix from canonic.toml, front matter, team closing.</span>
       <span class="cn-flow-cmd">{prefix}-*.md</span>
     </li>
     <li>
       <span class="cn-flow-n">2</span>
       <span class="cn-flow-title">Check quality</span>
       <span class="cn-flow-desc">Ids, prefix, sop, and generic closings - fail the PR early.</span>
       <span class="cn-flow-cmd">canonic check</span>
     </li>
     <li>
       <span class="cn-flow-n">3</span>
       <span class="cn-flow-title">Search &amp; dedupe</span>
       <span class="cn-flow-desc">Local Tantivy BM25 + Jaccard for near-copy discovery.</span>
       <span class="cn-flow-cmd">canonic search · dedupe</span>
     </li>
     <li>
       <span class="cn-flow-n">4</span>
       <span class="cn-flow-title">Publish to Jira</span>
       <span class="cn-flow-desc">Convert or explicit one-shot comment; imports stay drafts until promote.</span>
       <span class="cn-flow-cmd">canonic convert · jira-comment · tui</span>
     </li>
   </ol>

.. raw:: html

   <div class="cn-figure">
     <img src="_static/architecture.svg" width="840" height="280" alt="canonic architecture pipeline" />
     <p class="cn-figure-caption">Full pipeline diagram - details in <a href="architecture.html">Architecture</a>.</p>
   </div>

What you need → what you run
----------------------------

.. grid:: 1 2 2 2
   :gutter: 2

   .. grid-item-card:: Validate before merge
      :class-card: sd-border-0

      Front matter, configured prefix id, and team closings.

      .. raw:: html

         <ul class="cn-cmd-list"><li>canonic check<span class="cn-cmd-hint">exit 1 on findings</span></li></ul>

   .. grid-item-card:: Markdown → Jira wiki
      :class-card: sd-border-0

      Pandoc ``jira`` writer for a human to paste.

      .. raw:: html

         <ul class="cn-cmd-list"><li>canonic convert PATH</li></ul>

   .. grid-item-card:: Find similar answers
      :class-card: sd-border-0

      BM25 search and near-duplicate pairs.

      .. raw:: html

         <ul class="cn-cmd-list"><li>canonic search "…"</li><li>canonic dedupe</li></ul>

   .. grid-item-card:: Pull existing Jira text
      :class-card: sd-border-0

      REST read path → drafts under ``corpus/imports/``.

      .. raw:: html

         <ul class="cn-cmd-list"><li>canonic import-jira "JQL"</li></ul>

Install
=======

.. code:: shell

   cargo install --git https://github.com/HaoZeke/canonic
   # or from a checkout:
   cargo build --release

**Requirements:** Rust 1.85+, `pandoc <https://pandoc.org/>`_ for ``convert``,
optional `Vale <https://vale.sh/>`_ for style lint (Harper is linked in-process).
Run ``canonic doctor`` to see what the environment can already do.

First canned response
=====================

.. raw:: html

   <ol class="cn-steps">
     <li>
       <span class="cn-step-n" aria-hidden="true">1</span>
       <div class="cn-step-body">
         <span class="cn-step-title">Scaffold</span>
         <span class="cn-step-desc"><code>canonic new "Example topic"</code> → <code>corpus/responses/resp-example-topic.md</code> (or edit the included demo <code>resp-demo-shared-quota.md</code>).</span>
       </div>
     </li>
     <li>
       <span class="cn-step-n" aria-hidden="true">2</span>
       <div class="cn-step-body">
         <span class="cn-step-title">Validate</span>
         <span class="cn-step-desc"><code>canonic check</code></span>
       </div>
     </li>
     <li>
       <span class="cn-step-n" aria-hidden="true">3</span>
       <div class="cn-step-body">
         <span class="cn-step-title">Index &amp; search</span>
         <span class="cn-step-desc"><code>canonic reindex</code> then <code>canonic search "example topic"</code></span>
       </div>
     </li>
     <li>
       <span class="cn-step-n" aria-hidden="true">4</span>
       <div class="cn-step-body">
         <span class="cn-step-title">Convert</span>
         <span class="cn-step-desc">Needs pandoc: <code>canonic convert corpus/responses/resp-demo-shared-quota.md</code></span>
       </div>
     </li>
   </ol>

Walk the demos end-to-end: :doc:`tutorial`. Full command reference: :doc:`usage`.

Documentation map
=================

.. grid:: 1 2 2 2
   :gutter: 2

   .. grid-item-card:: Tutorial
      :link: tutorial
      :link-type: doc

      One path from checkout to convert using the included demos.

   .. grid-item-card:: Usage
      :link: usage
      :link-type: doc

      CLI overview, corpus layout, dedupe, Jira import, exit codes.

   .. grid-item-card:: Design
      :link: design
      :link-type: doc

      Why Tantivy, markdown-as-source, and the review-before-migrate rule.

   .. grid-item-card:: Architecture
      :link: architecture
      :link-type: doc

      Pipeline diagram, layers, and library module map.

   .. grid-item-card:: Rust API
      :link: api
      :link-type: doc

      Module map and embedded crate reference (sphinxcontrib-rust).

.. toctree::
   :maxdepth: 1
   :caption: Guides
   :hidden:

   tutorial
   usage
   design
   architecture
   api

Source & license
================

- Repository: https://github.com/HaoZeke/canonic
- License: MIT - see ``LICENSE``
- Site theme: `Shibuya <https://shibuya.lepture.com/>`_ (build with ``./docs/build.sh``)

#+end_export
